...
// 搜尋標案
export async function searchTenders(keyword: string, onlyActive: boolean, limit: number): Promise<string> {
...
// 依分類搜尋
export async function searchByCategory(categoryCode: string, keyword: string | undefined, limit: number): Promise<string> {
  // 找出分類名稱
...
// 取得標案詳情
export async function getTenderDetail(unitId: string, jobNumber: string): Promise<string> {
  const url = `${API_BASE}/tender?unit_id=${encodeURIComponent(unitId)}&job_number=${encodeURIComponent(jobNumber)}`;

  const response = await axios.get(url, { timeout: 10000 });
  const history = response.data.records as any[];
...
export async function listByDate(
  ...
  if (region) filterDesc.push(`地區：${region}`);
  if (keyword) filterDesc.push(`關鍵字：${keyword}`);
  if (typeFilter) filterDesc.push(`類型：${typeFilter}`);
...
// 依機關列出標案
export async function listByUnit(unitId: string, limit: number): Promise<string> {
...
// 列出分類
export async function getCategories(type: string, search: string | undefined): Promise<string> {
